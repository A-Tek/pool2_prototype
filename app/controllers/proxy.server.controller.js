...
function getApiParams(sourceId, recordId, searchTerm, currentPage)
{
    ...
    switch (sourceId) {
        case "fda":
            options = {
                url: recordId.length == 0 ? 'https://api.fda.gov/drug/label.json?api_key=A8kduY78n9q3U5kxQahgjp9QIoGbCU7VZfJGnqkm&search=' + searchTerm + '&skip=' + (currentPage == 1 ? '0' : nextRecordset.toString()) + '&limit=' + maxRecordsPerPage.toString() : 'https://api.fda.gov/drug/label.json?api_key=ohV19XYnKxoOarXUZlqdqaf9H1zwfpLk5XuXzpRp&search=openfda.spl_set_id:"'+ recordId +'"',
